...
ACHIEVEMENTS = [
    ...
    {"id": "casino_primeira_aposta", "name": "A Casa Sorriu", "desc": "Faça sua primeira aposta no Fallen Angel. O carpete já sabe seu nome.", "metric": "casino_games_played", "target": 1, "reward": {"gold": 300}},
    {"id": "casino_vitorias_10", "name": "Sortudo com Recibo", "desc": "Vença 10 apostas no cassino.", "metric": "casino_wins", "target": 10, "reward": {"gems": 25}},
    {"id": "casino_vitorias_100", "name": "Consultor da Banca", "desc": "Vença 100 apostas no cassino. O TutoriUAU está conferindo os dados duas vezes.", "metric": "casino_wins", "target": 100, "reward": {"gems": 140, "items": {"token_titulo_o_apostador": 1}}},
    {"id": "casino_numero_roleta", "name": "Eu Calculei, Confia", "desc": "Acerte uma aposta direta em número na roleta.", "metric": "casino_roulette_exact", "target": 1, "reward": {"gems": 80}},
    {"id": "casino_blackjack_natural", "name": "Vinte e Um com Pose", "desc": "Faça um blackjack natural.", "metric": "casino_blackjack_natural", "target": 1, "reward": {"gems": 60}},
    {"id": "casino_jackpot", "name": "Quebrou a Banca", "desc": "Ganhe um jackpot no caça-níquel.", "metric": "casino_jackpots", "target": 1, "reward": {"gems": 250, "items": {"token_titulo_quebrou_a_banca": 1}}},
    {"id": "casino_azarado_10", "name": "Tutorial de Derrota", "desc": "Perca 10 apostas seguidas. Impressionante, da forma estatisticamente triste.", "metric": "casino_loss_streak_best", "target": 10, "reward": {"gold": 500, "items": {"token_titulo_filho_do_rng": 1}}, "secret": True},
    {"id": "casino_loja_3", "name": "Comprou o Cinzeiro", "desc": "Compre 3 itens na loja do cassino.", "metric": "casino_shop_purchases", "target": 3, "reward": {"gems": 40}},
    {"id": "biblioteca_primeiro_acerto", "name": "Abriu o Livro Certo", "desc": "Acerte sua primeira pergunta na Biblioteca Perdida. O TutoriUAU anotou sem parecer orgulhoso.", "metric": "biblioteca_correct", "target": 1, "reward": {"gold": 400}},
    {"id": "biblioteca_100_acertos", "name": "Enciclopédia com Pernas", "desc": "Acerte 100 perguntas da Biblioteca Perdida.", "metric": "biblioteca_correct", "target": 100, "reward": {"gems": 120, "items": {"token_titulo_enciclopedia_ambulante": 1}}},
    {"id": "biblioteca_combo_10", "name": "Dez Páginas Sem Tropeçar", "desc": "Faça combo 10 na Biblioteca. A prateleira piscou respeito.", "metric": "biblioteca_best_combo", "target": 10, "reward": {"gems": 60}},
    {"id": "biblioteca_diaria_perfeita", "name": "Aluno Irritantemente Aplicado", "desc": "Faça uma diária perfeita na Biblioteca Perdida.", "metric": "biblioteca_daily_perfect", "target": 1, "reward": {"items": {"token_titulo_sabio_dos_ecos": 1}, "gems": 45}},
    {"id": "biblioteca_sem_dica", "name": "Sem Cola Gourmet", "desc": "Complete uma diária sem usar dica.", "metric": "biblioteca_daily_no_hint", "target": 1, "reward": {"gold": 2500}},
    {"id": "biblioteca_lendaria_rapida", "name": "Ctrl+F Humano", "desc": "Acerte uma pergunta lendária em até 30 segundos.", "metric": "biblioteca_legendary_fast", "target": 1, "reward": {"gems": 80}, "secret": True},
    {"id": "biblioteca_loja_3", "name": "Comprou Marcador de Página", "desc": "Compre 3 itens na loja da Biblioteca Perdida.", "metric": "biblioteca_shop_purchases", "target": 3, "reward": {"gems": 40}},
...
def collect_metrics(cursor, user_id):
    ...
    return {
        ...
        "casino_games_played": one(cursor, "SELECT games_played FROM casino_players WHERE user_id = ?", (user_id,)),
        "casino_wins": stats.get("casino_wins", 0),
        "casino_roulette_exact": stats.get("casino_roulette_exact", 0),
        "casino_blackjack_natural": stats.get("casino_blackjack_natural", 0),
        "casino_jackpots": stats.get("casino_jackpots", 0),
        "casino_loss_streak_best": stats.get("casino_loss_streak_best", 0),
        "casino_shop_purchases": stats.get("casino_shop_purchases", 0),
        "biblioteca_correct": one(cursor, "SELECT total_acertos FROM biblioteca_players WHERE user_id = ?", (user_id,)),
        "biblioteca_answers": one(cursor, "SELECT total_perguntas FROM biblioteca_players WHERE user_id = ?", (user_id,)),
        "biblioteca_pages": one(cursor, "SELECT paginas FROM biblioteca_players WHERE user_id = ?", (user_id,)),
        "biblioteca_best_combo": one(cursor, "SELECT maior_combo FROM biblioteca_players WHERE user_id = ?", (user_id,)),
        "biblioteca_daily_perfect": stats.get("biblioteca_daily_perfect", 0),
        "biblioteca_daily_no_hint": stats.get("biblioteca_daily_no_hint", 0),
        "biblioteca_legendary_fast": stats.get("biblioteca_legendary_fast", 0),
        "biblioteca_shop_purchases": stats.get("biblioteca_shop_purchases", 0),
