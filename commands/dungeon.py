...
# ==========================================
# FUNÇÕES UTILITÁRIAS LOCAIS E SEGURAS
# ==========================================
def _as_ratio(value, default_when_true=0.0):
    if value is True: return float(default_when_true)
    if value in (None, False): return 0.0
...
class DungeonBattleView(discord.ui.View):
    ...
    def _executar_habilidade(self, actor, skill_id):
        ...
        elif alvo_req in ["dps_aliado", "dps_aliados", "aliado_maior_atk"] and aliados: targets = [max(aliados, key=lambda x: x.get_stat("atk") + x.get_stat("matk"))]
    ...
    async def on_timeout(self):
        try: await self.message.edit(content="⏳ **A batalha expirou por inatividade.** O grupo recuou em segurança.", view=None)
        except: pass
...
class BossEncounterView(discord.ui.View):
    ...
    async def btn_enfrentar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        c = 2
        for in_id, quantidade in self.andar_data.get("boss_minions", {}).items():
            if isinstance(quantidade, list):
                quantidade = random.randint(quantidade[0], quantidade[1])
